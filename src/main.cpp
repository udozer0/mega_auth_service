...
http::response<http::string_body> dispatch( const http::request<http::string_body>& req )
{
     ...
     if ( path == "/health" )
          return handlers::health::handle( req );
     if ( path == "/openapi.yaml" )
          return handlers::docs::openapi_yaml();
     if ( path == "/docs" )
          return handlers::docs::swagger_ui();
     ...
     if ( path == "/_meta" )
     {
          ...
          r.body() = std::string( "{\"build\":\"" ) + BUILD_SIG + "\"," + "\"read_ms\":"
                     + std::to_string( READ_TIMEOUT_MS ) + "," + "\"idle_s\":" + std::to_string( IDLE_TIMEOUT_S ) + ","
     ...
     if ( path == "/auth/register" )
          return handlers::auth::auth_register( req );
     if ( path == "/auth/login" )
          return handlers::auth::auth_login( req );
     if ( path == "/auth/device-login" )
          return handlers::auth::auth_device_login( req );
     if ( path == "/auth/forgot" )
          return handlers::auth::auth_forgot( req );
     if ( path == "/auth/reset" )
          return handlers::auth::auth_reset( req );
     if ( path == "/auth/change-password" )
          return handlers::auth::auth_change_password( req );
     if ( path == "/auth/logout" )
          return handlers::auth::auth_logout( req );
     ...
     for ( ;; )
     {
          ...
          while ( true )
          {
               ...
               if ( header_raw.size() > header_limit )
               {
                    beast::error_code ig;
                    http::write(
                         stream, make_error( http::status::request_header_fields_too_large, "header too large" ), ig
                    );
                    return;
               }
          ...
          if ( MAX_BODY > 0 )
               parser.body_limit( ( std::uint64_t )MAX_BODY );
          if ( MAX_HDR > 0 )
               parser.header_limit( ( std::uint64_t )MAX_HDR );
          ...
          if ( ec )
          {
               std::cerr << "read_header(parse): " << ec.message() << "\n";
               return;
          }
               ...
               if ( might_have_body )
               {
                    ...
                    while ( !parser.is_done() )
                    {
                         ...
                         if ( READ_TIMEOUT_MS > 0 && remain <= 0 )
                         {
                              beast::error_code ig;
                              http::write(
                                   stream, make_error( http::status::request_timeout, "read timeout (body)" ), ig
                              );
                              return;
                         }
                         ...
                         if ( ec )
                         {
                              std::cerr << "read_some(body): " << ec.message() << "\n";
                              return;
                         }
                         buffer.commit( n );
                         http::read(
                              stream, buffer, parser, ec
                         ); // доедаем из buffer (read может не читать сокет, если буфер полон)
                         if ( ec == http::error::body_limit )
                         {
                              beast::error_code ig;
                              http::write(
                                   stream, make_error( http::status::payload_too_large, "body too large" ), ig
                              );
                              return;
                         }
                         if ( ec && ec != http::error::need_buffer )
                         {
                              std::cerr << "read(body): " << ec.message() << "\n";
                              return;
                         }
                    }
               }
          ...
          ++handled;
          if ( MAX_REQS > 0 && handled >= MAX_REQS )
               res.keep_alive( false );
